...
struct my_struct {
	...
	struct delayed_work delay_work; //kangta
	int			b;
} mydata;
...
{
	printk("This is a delay workqueue\r\n");
}
...
static int __init workqueue_init(void)
{
	...
	INIT_DELAYED_WORK(&mydata.delay_work, delay_work_run);//kangta
	schedule_work(&mydata.workqueue);
	schedule_delayed_work(&mydata.delay_work, 3 * HZ);//kangta
	return 0;
}
