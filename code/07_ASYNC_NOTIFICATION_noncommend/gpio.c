...
static struct work_struct set_kdb_led_init;
/////////////////////////////////////////////
...
struct file_operations my_fops = {
        ...
        open    :       my_open,
        release :       my_close,
		fasync  :       my_fasync, //be call, when "fcntl(fd, F_SETFL, fcntl(fd, F_GETFL) | FASYNC); in AP"
        owner   :       THIS_MODULE,
        };
...
static void set_kbd_led_do(struct work_struct *w)
{
...
static irqreturn_t r_irq_handler(int irq, void *dev_id, struct pt_regs *regs) 
{
	...
	if(delta_time < 3000){ // < 3000ms
		return IRQ_HANDLED;
	}
	
	time_stamp = now_time_val;
    printk("%s\r\n",__func__);
	schedule_work(&set_kdb_led_init);
   	return IRQ_HANDLED;
}
...
int init_module(void)
{
	...
	INIT_WORK(&set_kdb_led_init,set_kbd_led_do);
	r_int_config();
        return 0;
}
...
static ssize_t my_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
	...
	int minor;
	memset(msg, 0, 32);
	minor = MAJOR(filp->f_path.dentry->d_inode->i_rdev);			
...
static int my_close(struct inode *inod, struct file *fil)
{
	...
	int major;
	major = MAJOR(fil->f_path.dentry->d_inode->i_rdev);			
...
{
	if (!on)
		return fasync_remove_entry(filp, fapp); //移除
	return fasync_add_entry(fd, filp, fapp); //新增
}
*/
