...
#define DEVICE_NAME1            "chrauto1" //kangta
#define NUMBERS			2
#define BUF_SIZE			32
...
static struct class  *mydev_class1;//kangta
...
{
	dprint("device open\n");
	dprint("major = %d , minor = %d \n",imajor(inode),iminor(inode)); //kangta
...
{	
	...
	//step 3:cdev_add	
	if(cdev_add(&pmy_dev->cdev,mydev_num,2)) //kangta
	{
		dprint("Bad cdev\n");
		return 1;
	}
	...
	mydev_class = class_create(THIS_MODULE,DEVICE_NAME); // create class put in /sysfs
        mydev_class1 = class_create(THIS_MODULE,DEVICE_NAME1); //kangta
	//create /dev node and connect it to class that put in /sysfs
	device_create(mydev_class,NULL,mydev_num,NULL,DEVICE_NAME);
        device_create(mydev_class1,NULL,MKDEV(major_num,1),NULL,DEVICE_NAME1);//kangta
...
static void demo_exit(void) {
	...
	device_destroy(mydev_class,mydev_num);
        device_destroy(mydev_class1,MKDEV(major_num,1));//kangta  
	class_destroy(mydev_class); 
        class_destroy(mydev_class1);//kangta
